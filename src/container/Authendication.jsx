      ...
      <div className="w-full max-w-6xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col lg:flex-row transition-all duration-500">
        ...
        <div className={`lg:w-1/2  bg-[linear-gradient(rgba(15,23,42,0.75),rgba(15,23,42,0.75)),url('../assets/Images/authendication_img.jpg')]
         bg-cover bg-center bg-gradient-to-br  p-12 flex flex-col justify-between relative overflow-hidden
          ${type === 'signin' ? "translate-x-0" : "lg:translate-x-full"} transition-all duration-700 ease-in-out transform z-10 hidden min-[992px]:block`}>
          {/* Decorative Gears */}
          ...
          <div className="relative z-10 mt-12 flex items-center gap-6">
            <div className="flex -space-x-3">
              {[1, 2, 3].map((i) => (
                <div key={i} className="w-10 h-10 rounded-full border-2 border-[#0f172a] bg-gradient-to-br from-gray-600 to-gray-800"></div>
              ))}
            </div>
        ...
        {/* Right Side - Form */}
        <div className={`lg:w-1/2 p-8 lg:p-12  ${type === 'signin' ? "translate-x-0" : "lg:-translate-x-full"} transition-all duration-700 ease-in-out transform`}>
          {/* Tabs */}
          {
            type === 'signin' || type === 'signup' ?
              <div className="flex mb-10 bg-gray-100 p-1 rounded-xl">
                <button
                  ...
                  className={`flex-1 py-2 sm:py-3 px-4 rounded-lg font-bold transition-all ${type === 'signin'
                ...
                <button
                  ...
                  className={`flex-1 py-2 sm:py-3 px-4 rounded-lg font-bold transition-all ${type === 'signup'
                    ? "bg-[#f59e0b] text-[#0f172a] shadow-lg shadow-amber-500/20"
                    : "text-gray-500 hover:text-gray-700"
                    }`}
                >
              ...
              </div> : type === 'forgetpass' ? <h2 className="text-xl text-gray-500 mb-9">Forget Password</h2> : ''
          }
